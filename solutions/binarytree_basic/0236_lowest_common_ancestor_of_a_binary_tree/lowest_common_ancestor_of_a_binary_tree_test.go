...
// nodeVal 返回节点值，nil 节点返回 math.MinInt32 便于错误打印
...
func TestLowestCommonAncestor(t *testing.T) {
	tests := []struct {
		name string
		vals []int // 层序表示，math.MinInt32 为 nil
		p, q int   // 两个目标节点的值
		want int   // 最近公共祖先的值
	}{
		// LeetCode 官方示例
		{"官方示例1: p=5,q=1", []int{3, 5, 1, 6, 2, 0, 8, math.MinInt32, math.MinInt32, 7, 4}, 5, 1, 3},
		{"官方示例2: p=5,q=4", []int{3, 5, 1, 6, 2, 0, 8, math.MinInt32, math.MinInt32, 7, 4}, 5, 4, 5},
		{"官方示例3: p=1,q=2", []int{1, 2}, 1, 2, 1},

		// 边界：p 是 q 的祖先（公共祖先为 p 自身）
		{"p是q的祖先", []int{3, 5, 1, 6, 2, 0, 8, math.MinInt32, math.MinInt32, 7, 4}, 5, 7, 5},

		// 边界：q 是 p 的祖先（公共祖先为 q 自身）
		...
		// 边界：根节点本身就是公共祖先
		{"根为公共祖先", []int{3, 5, 1, 6, 2, 0, 8, math.MinInt32, math.MinInt32, 7, 4}, 7, 3, 3},

		// 边界：含负数的树
		{"含负数的树", []int{-1, -2, -3, -4, -5}, -4, -5, -2},

		// 边界：左斜链式树（4→3→2→1 全为左孩子）
		{"左斜链式树", []int{4, 3, math.MinInt32, 2, math.MinInt32, 1}, 1, 3, 3},

		// 边界：右斜链式树（1→2→3 全为右孩子）
		{"右斜链式树", []int{1, math.MinInt32, 2, math.MinInt32, 3}, 1, 3, 1},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if p == nil || q == nil {
				t.Fatalf("测试数据无效：未找到 p=%d 或 q=%d", tt.p, tt.q)
			}
			got := LowestCommonAncestor(root, p, q)
			if got == nil || got.Val != tt.want {
				t.Errorf("LowestCommonAncestor(p=%d, q=%d) = %v, want %d",
					tt.p, tt.q, nodeVal(got), tt.want)
			}
		})
	}
}
