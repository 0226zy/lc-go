...
// makeChain 生成 n 门课程的链式先修关系：0<-1<-2<-...<-n-1（无环）
...
// makeCycle 生成 n 门课程的环形先修关系（有环，无解）
...
func BenchmarkCanFinish(b *testing.B) {
	benchmarks := []struct {
		...
		{"100门链式无环", 100, makeChain(100)},
		{"1000门链式无环", 1000, makeChain(1000)},
		{"1000门成环", 1000, makeCycle(1000)},
	}
