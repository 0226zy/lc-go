...
回文串指正着读和反着读都一样的字符串，例如 `"aba"`、`"abba"`。如果存在多个长度相同的最长回文子串，返回其中任意一个即可。
...
换一个角度：回文串一定有一个「对称中心」。枚举每个字符（奇数长度回文）和每对相邻字符之间（偶数长度回文）作为中心，向两边扩展直到两端字符不相等，扩展出的最长串就是以该中心的最长回文。中心一共 `2n - 1` 个，每个中心最多扩展 O(n) 次，全程不需要额外数组，空间 O(1)。本题属于**「回文串中心扩展」**模板。
