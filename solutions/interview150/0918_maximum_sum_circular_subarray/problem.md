...
用 `[5,-3,5]` 验证执行过程：`total = 7`，`curMax` 的滚动轨迹为 `5 → 2 → 7`（得 `maxSum = 7`），`curMin` 的滚动轨迹为 `5 → -3 → 2`（得 `minSum = -3`），跨环候选 `total - minSum = 10`，答案 `max(7, 10) = 10`，与 DP 数组版一致。
